...
# ──────────────────────────────────────────────
# 服务管理封装（兼容 systemctl / Alpine service）
# ──────────────────────────────────────────────
_svc_restart() {
...
_apply_dns() {
  ...
  # 若 systemd-resolved 接管则不加锁（加了会冲突）
...
# ─── 安装 BBR Cloud 内核（ylx2016/kernel）───────────────────
...
# ─── 安装 BBRplus 新版（UJX6N/bbrplus-6.x_stable）──────────
...
case $bbr_opt in
# ═══ 内核安装 ═══════════════════════════════════════════════
1)  _bbr_install_bbr_cloud ;;           2)  _bbr_install_bbrplus_new ;;
3)  _bbr_install_xanmod main ;;         4)  _bbr_install_debian_cloud ;;
5)  _bbr_install_official_stable ;;     6)  _bbr_install_official_latest ;;
...
15) _bbr_ipv6_on ;; 16) _bbr_sysctl_merge ;;
17) _bbr_sysctl_edit ;;
...
if append_inbound "/usr/local/etc/xray/config.json" "$NEW_INBOUND" "$WS_PORT" "Xray"; then _svc_restart xray && _svc_enable xray >/dev/null 2>&1; _svc_is_active xray && SERVICE_STATUS="active" || SERVICE_STATUS="inactive"; else SERVICE_STATUS="config_error"; fi
else
CORE_NAME="Sing-box"
if ! command -v sing-box &> /dev/null; then echo -e "${YELLOW}   首次部署需下载 Sing-box 核心，请耐心等待...${NC}"; bash <(curl -fsSL https://sing-box.app/install.sh) > /dev/null 2>&1; hash -r; command -v sing-box &>/dev/null || { echo -e "\n${RED}[错误] Sing-box 核心下载失败，请检查网络连接。${NC}"; pause_for_enter; return; }; fi
...
if append_inbound "/usr/local/etc/xray/config.json" "$NEW_INBOUND" "$HY2_PORT" "Xray"; then _svc_restart xray && _svc_enable xray >/dev/null 2>&1; _svc_is_active xray && SERVICE_STATUS="active" || SERVICE_STATUS="inactive"; else SERVICE_STATUS="config_error"; fi
else
CORE_NAME="Sing-box"
if ! command -v sing-box &> /dev/null; then echo -e "${YELLOW}   首次部署需下载 Sing-box 核心，请耐心等待...${NC}"; bash <(curl -fsSL https://sing-box.app/install.sh) > /dev/null 2>&1; hash -r; command -v sing-box &>/dev/null || { echo -e "\n${RED}[错误] Sing-box 核心下载失败，请检查网络连接。${NC}"; pause_for_enter; return; }; fi
...
# ── 工具定义（普通数组，避免 bash 关联数组 local 作用域 bug）──
# 格式: 编号|命令|说明
local TOOL_LIST=(
