...
if [ "$qdisc_choice" == "2" ]; then qdisc="cake"; break; fi
  ...
  local w_ram
  w_ram=$(free -m | awk '/^Mem:/{print $2}')
  [ -z "$w_ram" ] || [ "$w_ram" -le 0 ] && w_ram=1024
  echo -e "\n${CYAN}>>> 系统自动探测内存: ${GREEN}${w_ram} MB${NC}"

  local qdisc_mode
  if [ "$latency" -le 120 ]; then
    qdisc_mode="cake"
    echo -e "${CYAN}[提示] 当前 RTT ≤ 120ms，网站非激进模式默认偏向 ${GREEN}CAKE${NC}。${NC}"
  else
    qdisc_mode="fq"
    echo -e "${CYAN}[提示] 当前 RTT > 120ms，网站非激进模式默认偏向 ${GREEN}FQ${NC}。${NC}"
  fi
  read -r -p "> 是否强制改写队列算法？(1: 保持网站默认, 2: 强制 FQ, 3: 强制 CAKE) [默认 1]: " qdisc_choice
  qdisc_choice="${qdisc_choice// /}"
...
PROFILE_JSON=$(LOCAL_BW="$local_bw" VPS_BW="$server_bw" LATENCY="$latency" MEM_MB="$w_ram" RAMP="$ramp_up" CC="$bbr_ver" QDISC="$qdisc" ECN="0" python3 - <<'PY'
import json, math, os
