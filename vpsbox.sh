...
_sync_shortcut_from_current() {
    ...
    local shortcut_ver=""
    [ -s "$SHORTCUT_PATH" ] && shortcut_ver=$(sed -n 's/^VPSBOX_VERSION="\([^"]*\)".*/\1/p' "$SHORTCUT_PATH" 2>/dev/null | head -1)
...
_check_startup_update() {
    ...
    remote_ver=$(curl -sL --connect-timeout 2 --max-time 3 "$SCRIPT_URL" 2>/dev/null | sed -n 's/^VPSBOX_VERSION="\([^"]*\)".*/\1/p' | head -1)
...
_ensure_hosts_hostname() {
  local current_hostname escaped_hostname
  [ -w /etc/hosts ] || return 0
  current_hostname=$(hostname 2>/dev/null || true)
  [ -z "$current_hostname" ] && return 0
  escaped_hostname=$(printf '%s' "$current_hostname" | sed 's/[][\\.^$*+?{}|()]/\\&/g')
  if ! grep -qE "^[[:space:]]*[0-9:.]+[[:space:]].*(^|[[:space:]])${escaped_hostname}([[:space:]]|$)" /etc/hosts 2>/dev/null; then
    echo "127.0.1.1 ${current_hostname}" >> /etc/hosts 2>/dev/null || true
  fi
}
_ensure_hosts_hostname
...
_svc_restart() {
  local systemctl_bin
  if systemctl_bin=$(_systemctl_bin); then "$systemctl_bin" restart "$1" && return 0; fi
  if command -v service >/dev/null 2>&1; then service "$1" restart
  else return 1; fi
}
_svc_start() {
  local systemctl_bin
  if systemctl_bin=$(_systemctl_bin); then "$systemctl_bin" start "$1" && return 0; fi
  if command -v service >/dev/null 2>&1; then service "$1" start
  else return 1; fi
}
_svc_stop() {
  local systemctl_bin
  if systemctl_bin=$(_systemctl_bin); then "$systemctl_bin" stop "$1" && return 0; fi
  if command -v service >/dev/null 2>&1; then service "$1" stop
  else return 1; fi
}
_svc_enable() {
  local systemctl_bin
  if systemctl_bin=$(_systemctl_bin); then "$systemctl_bin" enable "$1" && return 0; fi
  if command -v rc-update >/dev/null 2>&1; then rc-update add "$1" default
  elif command -v update-rc.d >/dev/null 2>&1; then update-rc.d "$1" defaults
  elif command -v chkconfig >/dev/null 2>&1; then chkconfig "$1" on
  else return 1; fi
}
_svc_is_active() {
  local systemctl_bin
  if systemctl_bin=$(_systemctl_bin); then timeout 5 "$systemctl_bin" is-active --quiet "$1" 2>/dev/null && return 0; fi
  if command -v service >/dev/null 2>&1; then timeout 5 service "$1" status &>/dev/null
  else return 1; fi
}

_svc_main_pid() {
  local systemctl_bin pid
  if systemctl_bin=$(_systemctl_bin); then
    pid=$(timeout 3 "$systemctl_bin" show -p MainPID "$1" 2>/dev/null | cut -d= -f2)
    [ -n "$pid" ] && [ "$pid" != "0" ] && { echo "$pid"; return 0; }
  fi
...
_svc_reload() {
  if _svc_is_active "$1" 2>/dev/null; then
    local OLD_PID systemctl_bin; OLD_PID=$(_svc_main_pid "$1")
    if systemctl_bin=$(_systemctl_bin); then
      if timeout 10 "$systemctl_bin" reload "$1" 2>/dev/null; then
        sleep 1
        if _svc_is_active "$1" 2>/dev/null; then
    ...
    if [ -n "$OLD_PID" ] && [ -n "$NEW_PID" ] && [ "$OLD_PID" != "$NEW_PID" ]; then
      ...
      echo -e "${YELLOW}[警告] $1 热重载失败，新配置未生效。请稍后手动重启该服务。${NC}"
    fi
    return 1
  else
...
_svc_delayed_restart() {
  ...
  if systemctl_bin=$(_systemctl_bin); then
    if command -v systemd-run >/dev/null 2>&1; then
      systemd-run --on-active="${delay}s" --unit="vpsbox-delayed-${service_name}-restart" "$systemctl_bin" restart "$service_name" >/dev/null 2>&1 && return 0
    fi
    ( sleep "$delay"; "$systemctl_bin" restart "$service_name" >/dev/null 2>&1 ) >/dev/null 2>&1 &
    return 0
  fi
  if command -v service >/dev/null 2>&1; then
...
_svc_daemon_reload() {
  local systemctl_bin
  if systemctl_bin=$(_systemctl_bin); then "$systemctl_bin" daemon-reload 2>/dev/null
  else return 0; fi
}
...
_read_domain_or_retry() {
  local __var="$1" prompt="$2" value
  while true; do
    read -r -p "$prompt" value
    value="${value// /}"
    [ "$value" = "0" ] && return 1
    if _valid_domain_name "$value"; then
      printf -v "$__var" '%s' "$value"
      return 0
    fi
    echo -e "${RED}[错误] 域名格式无效，请输入类似 example.com 的真实域名。${NC}"
  done
}
...
_bbr_install_official_latest() {
  ...
  if [[ "$BBR_OS_TYPE" == "CentOS" ]]; then
    ...
    if [[ "$BBR_OS_ID" == "ubuntu" || "$BBR_OS_ID" == "pop" || "$BBR_OS_LIKE" == *"ubuntu"* ]]; then
      ...
      [[ -z "$codename" ]] && codename=$(awk -F'[()]' '/^VERSION=/{print $2}' /etc/os-release | head -1)
...
if ! _valid_domain_name "$SNI_DOMAIN"; then echo -e "${RED}[错误] SNI 域名格式无效。${NC}"; pause_for_enter; return; fi
...
_read_domain_or_retry DOMAIN "> 请输入域名 (输入 0 取消): " || return
...
acquire_cert() {
    ...
    if ! _valid_domain_name "$DOMAIN"; then
        echo -e "\n${RED}[错误] 域名格式无效，已拒绝申请证书。${NC}"
        return 1
    fi
...
_cert_domains() {
  local d name
  {
    ...
    for d in /etc/vpsbox-cert/*; do
      [ -d "$d" ] && { [ -f "$d/fullchain.pem" ] || [ -f "$d/privkey.pem" ]; } && { name=$(basename "$d"); _valid_domain_name "$name" && echo "$name"; }
    done
    for d in /root/.acme.sh/*_ecc /root/.acme.sh/*; do
      [ -d "$d" ] || continue
      name=$(basename "$d")
      name="${name%_ecc}"
      [ -f "$d/${name}.cer" ] && _valid_domain_name "$name" && echo "$name"
    done
    shopt -u nullglob
  } | sort -u
}
...
if append_inbound "$(_config_file_for_core "$CORE_NAME")" "$NEW_INBOUND" "$WS_PORT" "$CORE_NAME" "WS-TLS" "vless-ws-tls" "$LINK"; then
    _ufw_allow_if_active "$WS_PORT" tcp
...
_setup_hy2_port_hopping() {
  ...
  if [ "$target_port" -ge "$start" ] && [ "$target_port" -le "$end" ]; then
    echo -e "${RED}[错误] 跳跃端口范围不能包含主监听端口 ${target_port}。${NC}"
    return 1
  fi
...
_ensure_hy2_self_signed_cert() {
  local sni="$1"
  local safe_sni
  safe_sni=$(_sanitize_filename_component "$sni")
  [ -z "$safe_sni" ] && safe_sni="selfsigned"
  CERT_DIR="/etc/vpsbox-cert/hy2-${safe_sni}"
  mkdir -p "$CERT_DIR" || return 1
  if [ -s "$CERT_DIR/fullchain.pem" ] && [ -s "$CERT_DIR/privkey.pem" ] && \
     openssl x509 -in "$CERT_DIR/fullchain.pem" -noout -dates >/dev/null 2>&1; then
...
echo -e "${CYAN}>>> 域名 / 证书模式${NC}"
echo -e "  ${GREEN}1.${NC} 无域名：使用自签证书，链接使用服务器 IP，客户端需允许不安全证书 ${CYAN}[简单]${NC}"
echo -e "  ${GREEN}2.${NC} 自有域名：申请 Let's Encrypt 证书，链接使用域名 ${CYAN}[正式]${NC}"
while true; do
read -r -p "> 请选择 [1-2, 默认 1, 0 取消]: " hy2_cert_profile
hy2_cert_profile="${hy2_cert_profile// /}"
[ "$hy2_cert_profile" = "0" ] && return
[ -z "$hy2_cert_profile" ] && hy2_cert_profile=1
case "$hy2_cert_profile" in
  1)
    DOMAIN=""
    DOMAIN_IP=""
  ...
  while true; do
    ...
    if [[ "$HY2_HOP_RANGE" =~ ^([0-9]+)-([0-9]+)$ ]]; then
      ...
      if _valid_port "$HOP_START" && _valid_port "$HOP_END" && [ "$HOP_START" -le "$HOP_END" ]; then
        if [ "$HY2_PORT" -ge "$HOP_START" ] && [ "$HY2_PORT" -le "$HOP_END" ]; then
          echo -e "${RED}[错误] 跳跃端口范围不能包含主监听端口 ${HY2_PORT}。${NC}"
          continue
        fi
        HY2_HOP_ENABLED=1
        break
...
if [ "$hy2_cert_profile" = "2" ]; then
  ...
  if ! confirm_action "开始部署无域名 Hysteria2 节点"; then pause_for_enter; return; fi
  install_dependencies
  _ensure_hy2_self_signed_cert "$HY2_SNI" || { echo -e "\n${RED}[错误] 自签证书生成失败。${NC}"; pause_for_enter; return; }
fi
HY2_PASS=$(openssl rand -hex 8)
if [ "$core_choice" == "1" ]; then
CORE_NAME="Xray"
ensure_xray_core || { pause_for_enter; return; }
NEW_INBOUND='{"listen":"0.0.0.0","port":'$HY2_PORT',"protocol":"hysteria","settings":{"version":2,"clients":[{"auth":"'$HY2_PASS'","level":0,"email":"vpsbox@hy2"}]},"streamSettings":{"network":"hysteria","security":"tls","tlsSettings":{"serverName":"'$HY2_SNI'","alpn":["h3"],"certificates":[{"certificateFile":"'$CERT_DIR'/fullchain.pem","keyFile":"'$CERT_DIR'/privkey.pem"}]},"hysteriaSettings":{"version":2}}}'
else
CORE_NAME="Sing-box"
ensure_singbox_core || { pause_for_enter; return; }
NEW_INBOUND='{"type":"hysteria2","listen":"0.0.0.0","listen_port":'$HY2_PORT',"users":[{"password":"'$HY2_PASS'"}],"tls":{"enabled":true,"server_name":"'$HY2_SNI'","alpn":["h3"],"certificate_path":"'$CERT_DIR'/fullchain.pem","key_path":"'$CERT_DIR'/privkey.pem"}}'
fi

LINK="hysteria2://${HY2_PASS}@${HY2_LINK_HOST}:${HY2_PORT}/?sni=${HY2_SNI}&alpn=h3&insecure=${HY2_INSECURE}"
...
_write_socat_forward() {
  local listen_port="$1" proto="$2" target_host="$3" target_port="$4" unit_file unit_name socat_bin
  unit_name=$(_forward_unit_name "$listen_port" "$proto")
  unit_file=$(_forward_unit_file "$listen_port" "$proto")
  socat_bin=$(command -v socat 2>/dev/null || echo /usr/bin/socat)
...
[Service]
Type=simple
ExecStart=${socat_bin} $([ "$proto" = "tcp" ] && printf 'TCP-LISTEN:%s,fork,reuseaddr TCP:%s:%s' "$listen_port" "$target_host" "$target_port" || printf 'UDP-LISTEN:%s,fork,reuseaddr UDP:%s:%s' "$listen_port" "$target_host" "$target_port")
Restart=always
RestartSec=3
LimitNOFILE=65535
...
_write_forward_state() {
  ...
  local q_target_host q_mode q_target_ip q_unit
  q_target_host=$(_shell_single_quote "$target_host")
  q_mode=$(_shell_single_quote "$mode")
  q_target_ip=$(_shell_single_quote "$target_ip")
  q_unit=$(_shell_single_quote "$unit")
  cat > "$(_forward_state_file "$listen_port" "$proto")" <<EOFSTATE
LISTEN_PORT=${listen_port}
PROTO=${proto}
TARGET_HOST=${q_target_host}
TARGET_PORT=${target_port}
MODE=${q_mode}
TARGET_IP=${q_target_ip}
UNIT=${q_unit}
EOFSTATE
  chmod 600 "$(_forward_state_file "$listen_port" "$proto")" 2>/dev/null || true
}
...
remote_ver=$(curl -sL --connect-timeout 2 --max-time 3 https://raw.githubusercontent.com/vmenzo/VPSBox/main/vpsbox.sh 2>/dev/null | sed -n 's/^VPSBOX_VERSION="\([^"]*\)".*/\1/p' | head -1)
...
if [ -s /tmp/vpsbox_update.sh ] && \
   grep -qE '^VPSBOX_VERSION="v?[0-9]+([.][0-9]+){1,3}"' /tmp/vpsbox_update.sh && \
   bash -n /tmp/vpsbox_update.sh; then
